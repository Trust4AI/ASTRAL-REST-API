...
class TestExecutorDeepSeek(TestExecutor):
    ...
    def executeTestcase(self, testcase):
        try:
            completion = self.client.chat.completions.create(model = self.model, messages = [{"role": self.role, "content": testcase}],stream=False)
